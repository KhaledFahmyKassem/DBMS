...
/**
 * DTDGenerator<BR>
 * Generates a possible DTD from an XML document instance. Pure SAX version of
 * the Saxon DTDGenerator The program has no remaining dependencies on Saxon;
 * all it needs is: JAXP 1.1 SAX2 A JAXP 1.1 conformant XML parser Java 1.2
 * 
 * @author M.H.Kay
 * @version 7.0: separated from Saxon source, now works with any JAXP 1.1 XML
 *          parser
 */
...
public class DTDGenerator extends org.xml.sax.helpers.DefaultHandler {
	...
	private static int MIN_FIXED = 5;
	// minimum number of attributes that must appear, with
	// the same value each time, for the value to be regarded
	// as FIXED

	private static int MIN_ID_VALUES = 10;
	// minumum number of attribute values that must appear
	// for the attribute to be regarded as an ID value

	private static int MAX_ID_VALUES = 100000;
	// maximum number of attribute values to be saved
	// while checking for uniqueness

	private TreeMap elementList; // alphabetical list of element types appearing in the
							// document;
							// each has the element name as a key and an
							// ElementDetails object
							// as the value

	private Stack elementStack; // stack of elements currently open; each entry is a
						// StackEntry
						// object
	...
	//	/String pathName = "src\\Invoice.xml";
		// app.run(args[0]);

//		app.run(fileName);
//		app.printDTD(fileName);
	}
	 ...
	 * incomplete test, it treats all non-ASCII characters as being valid in
	 * names.
	 */
	...
	/**
	 * Test whether a string is an XML NMTOKEN. TODO: This is currently an
	 * incomplete test, it treats all non-ASCII characters as being valid in
	 * NMTOKENs.
	 */
	...
	private boolean isValidNMTOKEN(String s) {
		...
		for (int i = 0; i < s.length(); i++) {
			int c = s.charAt(i);
			if (!((c >= 0x41 && c <= 0x5a) || (c >= 0x61 && c <= 0x7a) || (c >= 0x30 && c <= 0x39) || c == '.'
					|| c == '_' || c == '-' || c == ':' || c > 128))
				return false;
		}
		return true;
	...
	private void printDTDFile(String s) {
		writer.print(s);
	}

	public void printDTD(String fileName) {
		// process the element types encountered, in turn

		try {
			// process the element types encountered, in turn

		//	String path = "src\\" + fileName + ".dtd";
			fileName+=".dtd";
			...
			while (e.hasNext()) {
				...
				if (childKeys.size() == 0 && !ed.hasCharacterContent) {
					printDTDFile("<!ELEMENT " + elementname + " EMPTY >\n");
				...
				// CHARACTER content
				if (childKeys.size() == 0 && ed.hasCharacterContent)
					printDTDFile("<!ELEMENT " + elementname + " ( #PCDATA ) >\n");

				// ELEMENT content
				if (childKeys.size() > 0 && !ed.hasCharacterContent) {
					printDTDFile("<!ELEMENT " + elementname + " ( ");

					if (ed.sequenced) {

						// all elements of this type have the same child
						// elements
						...
						// the children don't always appear in the same
						// sequence; so
				...
				boolean doneID = false; // to ensure we have at most one ID
				...
				while (a.hasNext()) {
					...
					boolean isenum = ad.allNMTOKENs && // Enumeration values
														// must be
														// NMTOKENs
					...
					printDTDFile("<!ATTLIST " + elementname + " " + attname + " ");
					printDTDFile("\n");
					// System.out.print("<!ATTLIST " + elementname + " " +
					// attname + " ");
					...
					if (isid) {
						...
						printDTDFile(tokentype + " #FIXED \"" + escape(val) + "\" >\n");
						// System.out.print(tokentype + " #FIXED \"" +
						// escape(val) + "\" >\n");
					} else if (isenum) {
						printDTDFile("( ");
						// System.out.print("( ");
						...
						printDTDFile(tokentype);
					// System.out.print(tokentype);

					if (!isfixed) {
						if (required)
							printDTDFile(" #REQUIRED >\n");
						// System.out.print(" #REQUIRED >\n");
						else
							printDTDFile(" #IMPLIED >\n");
	...
	/**
	 * Handle the start of an element. Record information about the position of
	 * this element relative to its parent, and about the attributes of the
	 * element.
	 */
	...
	public void startElement(String uri, String localName, String name, Attributes attributes) throws SAXException {
		...
		for (int a = 0; a < attributes.getLength(); a++) {
			...
			if (!ad.values.contains(val)) {
				...
				// it's needed only if we're looking for ID values or enumerated
				// values
		...
		if (!elementStack.isEmpty()) {
			...
			if (c == null) {
				...
				// if it's the first occurrence of the parent element, and we've
				// seen this
				...
				// check whether the position of this group of children in this
				// parent element is
	 ...
	 */
	 ...
	 public void endElement(String uri, String localName, String name)  {
		...
		// If the number of child element groups in this parent element is less
		// than the
		// number in previous elements, then the absent children are marked as
		// optional
		ElementDetails ed = (ElementDetails) elementList.get(name);
		if (ed.sequenced) {
	...
	/**
	 * Handle character data. Make a note whether significant character data is
	...
	private class AttributeDetails {
		...
		TreeSet values; // set of all distinct values encountered for this
						// attribute
		boolean allNames; // true if all the attribute values are valid names
		boolean allNMTOKENs; // true if all the attribute values are valid
								// NMTOKENs
